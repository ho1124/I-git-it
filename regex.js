...
var regex = {
    ...
    max_length: function(selector, len, msg) {
        let target1 = $(selector);
        if(target1.val().length > len){
            alert(msg);
            target1.focus();
            return false;
        }
    ...
    field: function(selector, msg, regex_expr) {
        let target4 = $(selector);

        if(!target4.val() || !regex_expr.test(target4.val())){
            alert(msg);
            target4.focus();
            return false;
        }
    ...
    email: function(selector, msg) {
        return this.field(selector, msg, /^[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/);
    },
    ...
    jumin: function(selector, msg) {
        return this.field(selector, msg , /^d{2}([0]\d|[1][0-2])([0][1-9]|[1-2]\d|[3][0-1])[-]*[1-4]\d{6}$/);
    },
    ...
    phone: function(selector, msg) {
        ...
        let target5 = $(selector);

        if(!target5.val() || (!check1.test(target5.val()) && !check2.test(target5.val()))){
            alert(msg);
            target5.focus();
            return false;
        }
